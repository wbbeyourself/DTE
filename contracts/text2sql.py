...
class QuestionLabel(int, Enum):
    ...
    @classmethod
    def abbr_to_name(cls, abbr: str):
        enum2abbr = {
            QuestionLabel.Null.name: 'O',
            QuestionLabel.Keyword.name: 'Key',
            QuestionLabel.Table.name: 'Tab',
            QuestionLabel.Column.name: 'Col',
            QuestionLabel.Value.name: 'Val',
            QuestionLabel.Ambiguity.name: 'Amb',
            QuestionLabel.Unknown.name: 'Unk',
        }
        if abbr in enum2abbr:
            return abbr
        
        abbr2enum = {v : k for k, v in enum2abbr.items()}
        if abbr in abbr2enum:
            enum_name = abbr2enum[abbr]
            return enum_name
        else:
            raise ValueError(f"invalid abbr : {abbr}")
    
    @classmethod
    def get_abbr_by_name(cls, name: str):
        enum2abbr = {
            QuestionLabel.Null: 'O',
            QuestionLabel.Keyword: 'Key',
            QuestionLabel.Table: 'Tab',
        ...
        for one in QuestionLabel.get_all_labels():
            if one.name == name:
                return enum2abbr[one]
        raise ValueError(f'unknown question label name : {name}')
...
class Text2SQLExample(JsonSerializable):
    ...
    def from_json(cls, obj: Dict):
        ...
        obj['matched_values'] = [CellValue.from_json(x) for x in obj['matched_values']] if obj['matched_values'] is not None else None
        obj['erased_ngrams'] = [Span.from_json(x) for x in obj['erased_ngrams']] if obj['erased_ngrams'] is not None else None
        ...
        obj['binding_result'] = [BindingItem.from_json(x) for x in obj['binding_result']] if obj['binding_result'] is not None else None
        if 'uuid' in obj:
            obj['uuid'] = obj['uuid']
        else:
            obj['uuid'] = 'uuid-hello-world'
        obj['unique_str'] = obj['unique_str'] if 'unique_str' in obj else 'i-love-you'
    ...
    def to_json(self) -> Dict:
        ...
        js['uuid'] = self.uuid
        js['unique_str'] = self.unique_str
        js['value_resolved'] = self.value_resolved
        js['language'] = str(self.language)
        return js
    
    def get_binding_result_json_list(self) -> Dict:
        js_list = []
        for item in self.binding_result:
            js_list.append(item.to_json())
        return js_list
    ...
    def ignore_unmatched_values(self):
        ...
        len_tokens = len(self.question.tokens)
        # warning invalid span index
        filter_values = []
        for item in self.matched_values:
            if item.span.start >= len_tokens or item.span.end >= len_tokens:
                print('error span in matched_values !!!!!!!!! ')
                print(f"self.question.text: {self.question.text}")
                print(f"self.uuid: {self.uuid}")
                print(f"len_tokens : {len_tokens}, invalid span : {item.span}")
                print()
                continue
            else:
                filter_values.append(item)
        
        self.matched_values = filter_values
        return self
    ...
    def get_concept_labels(self) -> Dict[SQLTokenType, List[int]]:
        ...
        for column in self.schema.columns:
            col_tokens = [sql_token for sql_token in self.sql.tokens if sql_token.type == SQLTokenType.Column and sql_token.column == column.identifier]
            column_labels += [len(col_tokens)]

        for value in self.matched_values:
            val_tokens = [sql_token for sql_token in self.sql.tokens if sql_token.type == SQLTokenType.Value and self.are_equal_values(sql_token, value)]
            value_labels += [len(val_tokens)]

        for keyword in All_Agg_Op_Keywords:
            valid_tokens = [sql_token for sql_token in self.sql.tokens if sql_token.type == SQLTokenType.Keyword and keyword.lower() == sql_token.value.lower()]
            keyword_labels += [len(valid_tokens)]

        return { SQLTokenType.Column: column_labels, SQLTokenType.Value: value_labels, SQLTokenType.Keyword: keyword_labels }

    def are_equal_values(self, sql_value: SQLToken, matched_value: CellValue) -> bool:
        matched_value_str = matched_value.name
        return sql_value.cell_value.replace('"', '').replace(" ", "").lower() == (Concept_Sep.join([matched_value.column, matched_value_str])).replace(" ", "").lower()
...
class ChaseExample(Text2SQLExample):
    ...
    def get_concept_labels(self):
        column_labels = []
        for column in self.schema.columns:
            col_tokens = [x.entity for x in self.schema_linkings if x.entity.type == SQLTokenType.Column and x.entity.value == column.identifier]
            fields = set([token.field for token in col_tokens])
            column_labels += [{'fields': fields, 'funcs': [] }]

        table_labels = []
        for table in self.schema.tables:
            tbl_tokens = [x.entity for x in self.schema_linkings if x.entity.type == SQLTokenType.Table and x.entity.value == table.identifier]
        ...
        return { SQLTokenType.Column: column_labels, SQLTokenType.Value: [], SQLTokenType.Table: table_labels,  SQLTokenType.Keyword: keyword_labels }